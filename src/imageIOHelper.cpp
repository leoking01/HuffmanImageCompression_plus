...
Image* ImageIOHelper::readImage(const string& filename) {
    ...
    for( int i=0;i<BitmapInfoLength ;i++  )
    {
    	printf( "imageInfoPtr[ %d ] = %d  \n ",i,   imageInfoPtr[i]  );
    }
...
//    if( rawSize == 0 )
//    	rawSize = dataSize;
    rawSize = imageInfoPtr[5]*pow(256, 3)+  imageInfoPtr[4]*pow(256, 2)
    		+  imageInfoPtr[3]*pow(256, 1)+  imageInfoPtr[2]*pow(256, 0);

    printf("width , height , rawSize ,   dataSize = %d , %d, %d,  %d \n",
    		width, height , rawSize ,   dataSize );
